	...
	for each (seg in msg.AIP) {
		...
		if (aprGroup.length > 0) {
			...
			for each (aprSeg in aprGroup) {
				aip[index].apr[indexAPR] = {};

				// APR.1, timeSelectionCriteria
				aip[index].apr[indexAPR].timeSelectionCriteria = [];
				for each (field in aprSeg['APR.1']) {
					...
					if (Object.keys(block).length > 0) {
						aip[index].apr[indexAPR].timeSelectionCriteria.push(block);
					}
				}
				if (aip[index].apr[indexAPR].timeSelectionCriteria.length == 0) {
					delete aip[index].apr[indexAPR].timeSelectionCriteria;
				}

				// APR.2, resourceSelectionCriteria
				aip[index].apr[indexAPR].resourceSelectionCriteria = [];
				for each (field in aprSeg['APR.2']) {
					...
					if (Object.keys(block).length > 0) {
						aip[index].apr[indexAPR].resourceSelectionCriteria.push(block);
					}
				}
				if (aip[index].apr[indexAPR].resourceSelectionCriteria.length == 0) {
					delete aip[index].apr[indexAPR].resourceSelectionCriteria;
				}

				// APR.3, locationSelectionCriteria
				aip[index].apr[indexAPR].locationSelectionCriteria = [];
				for each (field in aprSeg['APR.3']) {
					...
					if (Object.keys(block).length > 0) {
						aip[index].apr[indexAPR].locationSelectionCriteria.push(block);
					}
				}
				if (aip[index].apr[indexAPR].locationSelectionCriteria.length == 0) {
					delete aip[index].apr[indexAPR].locationSelectionCriteria;
				}

				// APR.4, slotSpacingCriteria
				if (aprSeg['APR.4']['APR.4.1'].toString() != "") { aip[index].apr[indexAPR].slotSpacingCriteria = aprSeg['APR.4']['APR.4.1']; }

				// APR.5, fillerOverrideCriteria
				aip[index].apr[indexAPR].fillerSelectionCriteria = [];
				for each (field in aprSeg['APR.5']) {
					...
					if (Object.keys(block).length > 0) {
						aip[index].apr[indexAPR].fillerSelectionCriteria.push(block);
					}
				}
				if (aip[index].apr[indexAPR].fillerSelectionCriteria.length == 0) {
					delete aip[index].apr[indexAPR].fillerSelectionCriteria;
				}

				// clean up APR and increment index
				if (Object.keys(aip[index].apr[indexAPR]).length == 0) {
					delete aip[index].apr[indexAPR];
				} else {
					indexAPR++;
				}
			}
