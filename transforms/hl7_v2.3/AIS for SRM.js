	...
	for each (seg in msg.AIS) {
		...
		if (aprGroup.length > 0) {
			...
			for each (aprSeg in aprGroup) {
				ais[index].apr[indexAPR] = {};

				// APR.1, timeSelectionCriteria
				ais[index].apr[indexAPR].timeSelectionCriteria = [];
				for each (field in aprSeg['APR.1']) {
					...
					if (Object.keys(block).length > 0) {
						ais[index].apr[indexAPR].timeSelectionCriteria.push(block);
					}
				}
				if (ais[index].apr[indexAPR].timeSelectionCriteria.length == 0) {
					delete ais[index].apr[indexAPR].timeSelectionCriteria;
				}

				// APR.2, resourceSelectionCriteria
				ais[index].apr[indexAPR].resourceSelectionCriteria = [];
				for each (field in aprSeg['APR.2']) {
					...
					if (Object.keys(block).length > 0) {
						ais[index].apr[indexAPR].resourceSelectionCriteria.push(block);
					}
				}
				if (ais[index].apr[indexAPR].resourceSelectionCriteria.length == 0) {
					delete ais[index].apr[indexAPR].resourceSelectionCriteria;
				}

				// APR.3, locationSelectionCriteria
				ais[index].apr[indexAPR].locationSelectionCriteria = [];
				for each (field in aprSeg['APR.3']) {
					...
					if (Object.keys(block).length > 0) {
						ais[index].apr[indexAPR].locationSelectionCriteria.push(block);
					}
				}
				if (ais[index].apr[indexAPR].locationSelectionCriteria.length == 0) {
					delete ais[index].apr[indexAPR].locationSelectionCriteria;
				}

				// APR.4, slotSpacingCriteria
				if (aprSeg['APR.4']['APR.4.1'].toString() != "") { ais[index].apr[indexAPR].slotSpacingCriteria = aprSeg['APR.4']['APR.4.1']; }

				// APR.5, fillerOverrideCriteria
				ais[index].apr[indexAPR].fillerSelectionCriteria = [];
				for each (field in aprSeg['APR.5']) {
					...
					if (Object.keys(block).length > 0) {
						ais[index].apr[indexAPR].fillerSelectionCriteria.push(block);
					}
				}
				if (ais[index].apr[indexAPR].fillerSelectionCriteria.length == 0) {
					delete ais[index].apr[indexAPR].fillerSelectionCriteria;
				}

				// clean up APR and increment index
				if (Object.keys(ais[index].apr[indexAPR]).length == 0) {
					delete ais[index].apr[indexAPR];
				} else {
					indexAPR++;
				}
			}
