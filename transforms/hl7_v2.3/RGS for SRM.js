...
if (aisGroup.length > 0) {
	...
	for each (aisSeg in aisGroup) {
		...
		// APR (AIS)
		var aprGroup = getSegmentsAfter(msg,aisSeg,'APR',true);
		if (aprGroup.length > 0) {
			rgs.ais[indexAIS].apr = [];
			var indexAPR = 0;
			for each (aprSeg in aprGroup) {
				rgs.ais[indexAIS].apr[indexAPR] = {};

				// APR.1, timeSelectionCriteria
				rgs.ais[indexAIS].apr[indexAPR].timeSelectionCriteria = [];
				for each (field in aprSeg['APR.1']) {
					...
					if (Object.keys(block).length > 0) {
						rgs.ais[indexAIS].apr[indexAPR].timeSelectionCriteria.push(block);
					}
				}
				if (rgs.ais[indexAIS].apr[indexAPR].timeSelectionCriteria.length == 0) {
					delete rgs.ais[indexAIS].apr[indexAPR].timeSelectionCriteria;
				}

				// APR.2, resourceSelectionCriteria
				rgs.ais[indexAIS].apr[indexAPR].resourceSelectionCriteria = [];
				for each (field in aprSeg['APR.2']) {
					...
					if (Object.keys(block).length > 0) {
						rgs.ais[indexAIS].apr[indexAPR].resourceSelectionCriteria.push(block);
					}
				}
				if (rgs.ais[indexAIS].apr[indexAPR].resourceSelectionCriteria.length == 0) {
					delete rgs.ais[indexAIS].apr[indexAPR].resourceSelectionCriteria;
				}

				// APR.3, locationSelectionCriteria
				rgs.ais[indexAIS].apr[indexAPR].locationSelectionCriteria = [];
				for each (field in aprSeg['APR.3']) {
					...
					if (Object.keys(block).length > 0) {
						rgs.ais[indexAIS].apr[indexAPR].locationSelectionCriteria.push(block);
					}
				}
				if (rgs.ais[indexAIS].apr[indexAPR].locationSelectionCriteria.length == 0) {
					delete rgs.ais[indexAIS].apr[indexAPR].locationSelectionCriteria;
				}

				// APR.4, slotSpacingCriteria
				if (aprSeg['APR.4']['APR.4.1'].toString() != "") { rgs.ais[indexAIS].apr[indexAPR].slotSpacingCriteria = aprSeg['APR.4']['APR.4.1']; }

				// APR.5, fillerOverrideCriteria
				rgs.ais[indexAIS].apr[indexAPR].fillerSelectionCriteria = [];
				for each (field in aprSeg['APR.5']) {
					var block = {};
					if (field['APR.5.1'].toString() != "") { block.parameterClass = field['APR.5.1']; }
					if (field['APR.5.2'].toString() != "") { block.parameterValue = field['APR.5.2']; }
					if (Object.keys(block).length > 0) {
						rgs.ais[indexAIS].apr[indexAPR].fillerSelectionCriteria.push(block);
					}
				}
				if (rgs.ais[indexAIS].apr[indexAPR].fillerSelectionCriteria.length == 0) {
					delete rgs.ais[indexAIS].apr[indexAPR].fillerSelectionCriteria;
				}

				// clean up APR and increment index
				if (Object.keys(rgs.ais[indexAIS].apr[indexAPR]).length == 0) {
					delete rgs.ais[indexAIS].apr[indexAPR];
				} else {
					indexAPR++;
				}
			}
		...
		// clean up AIS and increment index
...
if (aigGroup.length > 0) {
	...
	for each (aigSeg in aigGroup) {
		...
		// APR (AIG)
		var aprGroup = getSegmentsAfter(msg,aigSeg,'APR',true);
		if (aprGroup.length > 0) {
			rgs.aig[indexAIG].apr = [];
			var indexAPR = 0;
			for each (aprSeg in aprGroup) {
				rgs.aig[indexAIG].apr[indexAPR] = {}

				// APR.1, timeSelectionCriteria
				rgs.aig[indexAIG].apr[indexAPR].timeSelectionCriteria = [];
				for each (field in aprSeg['APR.1']) {
					var block = {};
					if (field['APR.1.1'].toString() != "") { block.parameterClass = field['APR.1.1']; }
					if (field['APR.1.2'].toString() != "") { block.parameterValue = field['APR.1.2']; }
					if (Object.keys(block).length > 0) {
						rgs.aig[indexAIG].apr[indexAPR].timeSelectionCriteria.push(block);
					}
				}
				if (rgs.aig[indexAIG].apr[indexAPR].timeSelectionCriteria.length == 0) {
					delete rgs.aig[indexAIG].apr[indexAPR].timeSelectionCriteria;
				}

				// APR.2, resourceSelectionCriteria
				rgs.aig[indexAIG].apr[indexAPR].resourceSelectionCriteria = [];
				for each (field in aprSeg['APR.2']) {
					var block = {};
					if (field['APR.2.1'].toString() != "") { block.parameterClass = field['APR.2.1']; }
					if (field['APR.2.2'].toString() != "") { block.parameterValue = field['APR.2.2']; }
					if (Object.keys(block).length > 0) {
						rgs.aig[indexAIG].apr[indexAPR].resourceSelectionCriteria.push(block);
					}
				}
				if (rgs.aig[indexAIG].apr[indexAPR].resourceSelectionCriteria.length == 0) {
					delete rgs.aig[indexAIG].apr[indexAPR].resourceSelectionCriteria;
				}

				// APR.3, locationSelectionCriteria
				rgs.aig[indexAIG].apr[indexAPR].locationSelectionCriteria = [];
				for each (field in aprSeg['APR.3']) {
					var block = {};
					if (field['APR.3.1'].toString() != "") { block.parameterClass = field['APR.3.1']; }
					if (field['APR.3.2'].toString() != "") { block.parameterValue = field['APR.3.2']; }
					if (Object.keys(block).length > 0) {
						rgs.aig[indexAIG].apr[indexAPR].locationSelectionCriteria.push(block);
					}
				}
				if (rgs.aig[indexAIG].apr[indexAPR].locationSelectionCriteria.length == 0) {
					delete rgs.aig[indexAIG].apr[indexAPR].locationSelectionCriteria;
				}

				// APR.4, slotSpacingCriteria
				if (aprSeg['APR.4']['APR.4.1'].toString() != "") { rgs.aig[indexAIG].apr[indexAPR].slotSpacingCriteria = aprSeg['APR.4']['APR.4.1']; }

				// APR.5, fillerOverrideCriteria
				rgs.aig[indexAIG].apr[indexAPR].fillerSelectionCriteria = [];
				for each (field in seg['APR.5']) {
					var block = {};
					if (field['APR.5.1'].toString() != "") { block.parameterClass = field['APR.5.1']; }
					if (field['APR.5.2'].toString() != "") { block.parameterValue = field['APR.5.2']; }
					if (Object.keys(block).length > 0) {
						rgs.aig[indexAIG].apr[indexAPR].fillerSelectionCriteria.push(block);
					}
				}
				if (rgs.aig[indexAIG].apr[indexAPR].fillerSelectionCriteria.length == 0) {
					delete rgs.aig[indexAIG].apr[indexAPR].fillerSelectionCriteria;
				}

				// clean up APR and increment index
				if (Object.keys(rgs.aig[indexAIG].apr[indexAPR]).length == 0) {
					delete rgs.aig[indexAIG].apr[indexAPR];
				} else {
					indexAPR++;
				}
			}
...
if (ailGroup.length > 0) {
	...
	for each (ailSeg in ailGroup) {
		...
		// APR (AIL)
		var aprGroup = getSegmentsAfter(msg,ailSeg,'APR',true);
		if (aprGroup.length > 0) {
			rgs.ail[indexAIL].apr = [];
			var indexAPR = 0;
			for each (aprSeg in aprGroup) {
				rgs.ail[indexAIL].apr[indexAPR] = {};

				// APR.1, timeSelectionCriteria
				rgs.ail[indexAIL].apr[indexAPR].timeSelectionCriteria = [];
				for each (field in aprSeg['APR.1']) {
					var block = {};
					if (field['APR.1.1'].toString() != "") { block.parameterClass = field['APR.1.1']; }
					if (field['APR.1.2'].toString() != "") { block.parameterValue = field['APR.1.2']; }
					if (Object.keys(block).length > 0) {
						rgs.ail[indexAIL].apr[indexAPR].timeSelectionCriteria.push(block);
					}
				}
				if (rgs.ail[indexAIL].apr[indexAPR].timeSelectionCriteria.length == 0) {
					delete rgs.ail[indexAIL].apr[indexAPR].timeSelectionCriteria;
				}

				// APR.2, resourceSelectionCriteria
				rgs.ail[indexAIL].apr[indexAPR].resourceSelectionCriteria = [];
				for each (field in aprSeg['APR.2']) {
					var block = {};
					if (field['APR.2.1'].toString() != "") { block.parameterClass = field['APR.2.1']; }
					if (field['APR.2.2'].toString() != "") { block.parameterValue = field['APR.2.2']; }
					if (Object.keys(block).length > 0) {
						rgs.ail[indexAIL].apr[indexAPR].resourceSelectionCriteria.push(block);
					}
				}
				if (rgs.ail[indexAIL].apr[indexAPR].resourceSelectionCriteria.length == 0) {
					delete rgs.ail[indexAIL].apr[indexAPR].resourceSelectionCriteria;
				}

				// APR.3, locationSelectionCriteria
				rgs.ail[indexAIL].apr[indexAPR].locationSelectionCriteria = [];
				for each (field in aprSeg['APR.3']) {
					var block = {};
					if (field['APR.3.1'].toString() != "") { block.parameterClass = field['APR.3.1']; }
					if (field['APR.3.2'].toString() != "") { block.parameterValue = field['APR.3.2']; }
					if (Object.keys(block).length > 0) {
						rgs.ail[indexAIL].apr[indexAPR].locationSelectionCriteria.push(block);
					}
				}
				if (rgs.ail[indexAIL].apr[indexAPR].locationSelectionCriteria.length == 0) {
					delete rgs.ail[indexAIL].apr[indexAPR].locationSelectionCriteria;
				}

				// APR.4, slotSpacingCriteria
				if (aprSeg['APR.4']['APR.4.1'].toString() != "") { rgs.ail[indexAIL].apr[indexAPR].slotSpacingCriteria = aprSeg['APR.4']['APR.4.1']; }

				// APR.5, fillerOverrideCriteria
				rgs.ail[indexAIL].apr[indexAPR].fillerSelectionCriteria = [];
				for each (field in aprSeg['APR.5']) {
					var block = {};
					if (field['APR.5.1'].toString() != "") { block.parameterClass = field['APR.5.1']; }
					if (field['APR.5.2'].toString() != "") { block.parameterValue = field['APR.5.2']; }
					if (Object.keys(block).length > 0) {
						rgs.ail[indexAIL].apr[indexAPR].fillerSelectionCriteria.push(block);
					}
				}
				if (rgs.ail[indexAIL].apr[indexAPR].fillerSelectionCriteria.length == 0) {
					delete rgs.ail[indexAIL].apr[indexAPR].fillerSelectionCriteria;
				}

				// clean up APR and increment index
				if (Object.keys(rgs.ail[indexAIL].apr[indexAPR]).length == 0) {
					delete rgs.ail[indexAIL].apr[indexAPR];
				} else {
					indexAPR++;
				}
			}
...
if (aiProup.length > 0) {
	...
	for each (aipSeg in aipGroup) {
		...
		// APR (AIP)
		var aprGroup = getSegmentsAfter(msg,aipSeg,'APR',true);
		if (aprGroup.length > 0) {
			rgs.aip[indexAIP].apr = [];
			var indexAPR = 0;
			for each (aprSeg in aprGroup) {
				rgs.aip[indexAIP].apr[indexAPR] = {};

				// APR.1, timeSelectionCriteria
				rgs.aip[indexAIP].apr[indexAPR].timeSelectionCriteria = [];
				for each (field in aprSeg['APR.1']) {
					var block = {};
					if (field['APR.1.1'].toString() != "") { block.parameterClass = field['APR.1.1']; }
					if (field['APR.1.2'].toString() != "") { block.parameterValue = field['APR.1.2']; }
					if (Object.keys(block).length > 0) {
						rgs.aip[indexAIP].apr[indexAPR].timeSelectionCriteria.push(block);
					}
				}
				if (rgs.aip[indexAIP].apr[indexAPR].timeSelectionCriteria.length == 0) {
					delete rgs.aip[indexAIP].apr[indexAPR].timeSelectionCriteria;
				}

				// APR.2, resourceSelectionCriteria
				rgs.aip[indexAIP].apr[indexAPR].resourceSelectionCriteria = [];
				for each (field in aprSeg['APR.2']) {
					var block = {};
					if (field['APR.2.1'].toString() != "") { block.parameterClass = field['APR.2.1']; }
					if (field['APR.2.2'].toString() != "") { block.parameterValue = field['APR.2.2']; }
					if (Object.keys(block).length > 0) {
						rgs.aip[indexAIP].apr[indexAPR].resourceSelectionCriteria.push(block);
					}
				}
				if (rgs.aip[indexAIP].apr[indexAPR].resourceSelectionCriteria.length == 0) {
					delete rgs.aip[indexAIP].apr[indexAPR].resourceSelectionCriteria;
				}

				// APR.3, locationSelectionCriteria
				rgs.aip[indexAIP].apr[indexAPR].locationSelectionCriteria = [];
				for each (field in aprSeg['APR.3']) {
					var block = {};
					if (field['APR.3.1'].toString() != "") { block.parameterClass = field['APR.3.1']; }
					if (field['APR.3.2'].toString() != "") { block.parameterValue = field['APR.3.2']; }
					if (Object.keys(block).length > 0) {
						rgs.aip[indexAIP].apr[indexAPR].locationSelectionCriteria.push(block);
					}
				}
				if (rgs.aip[indexAIP].apr[indexAPR].locationSelectionCriteria.length == 0) {
					delete rgs.aip[indexAIP].apr[indexAPR].locationSelectionCriteria;
				}

				// APR.4, slotSpacingCriteria
				if (aprSeg['APR.4']['APR.4.1'].toString() != "") { rgs.aip[indexAIP].apr[indexAPR].slotSpacingCriteria = aprSeg['APR.4']['APR.4.1']; }

				// APR.5, fillerOverrideCriteria
				rgs.aip[indexAIP].apr[indexAPR].fillerSelectionCriteria = [];
				for each (field in aprSeg['APR.5']) {
					var block = {};
					if (field['APR.5.1'].toString() != "") { block.parameterClass = field['APR.5.1']; }
					if (field['APR.5.2'].toString() != "") { block.parameterValue = field['APR.5.2']; }
					if (Object.keys(block).length > 0) {
						rgs.aip[indexAIP].apr[indexAPR].fillerSelectionCriteria.push(block);
					}
				}
				if (rgs.aip[indexAIP].apr[indexAPR].fillerSelectionCriteria.length == 0) {
					delete rgs.aip[indexAIP].apr[indexAPR].fillerSelectionCriteria;
				}

				// clean up APR and increment index
				if (Object.keys(rgs.aip[indexAIP].apr[indexAPR]).length == 0) {
					delete rgs.aip[indexAIP].apr[indexAPR];
				} else {
					indexAPR++;
				}
			}
