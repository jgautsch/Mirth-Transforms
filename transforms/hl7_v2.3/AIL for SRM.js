	...
	for each (seg in msg.AIL) {
		...
		if (aprGroup.length > 0) {
			...
			for each (aprSeg in aprGroup) {
				ail[index].apr[indexAPR] = {};

				// APR.1, timeSelectionCriteria
				ail[index].apr[indexAPR].timeSelectionCriteria = [];
				for each (field in aprSeg['APR.1']) {
					...
					if (Object.keys(block).length > 0) {
						ail[index].apr[indexAPR].timeSelectionCriteria.push(block);
					}
				}
				if (ail[index].apr[indexAPR].timeSelectionCriteria.length == 0) {
					delete ail[index].apr[indexAPR].timeSelectionCriteria;
				}

				// APR.2, resourceSelectionCriteria
				ail[index].apr[indexAPR].resourceSelectionCriteria = [];
				for each (field in aprSeg['APR.2']) {
					...
					if (Object.keys(block).length > 0) {
						ail[index].apr[indexAPR].resourceSelectionCriteria.push(block);
					}
				}
				if (ail[index].apr[indexAPR].resourceSelectionCriteria.length == 0) {
					delete ail[index].apr[indexAPR].resourceSelectionCriteria;
				}

				// APR.3, locationSelectionCriteria
				ail[index].apr[indexAPR].locationSelectionCriteria = [];
				for each (field in aprSeg['APR.3']) {
					...
					if (Object.keys(block).length > 0) {
						ail[index].apr[indexAPR].locationSelectionCriteria.push(block);
					}
				}
				if (ail[index].apr[indexAPR].locationSelectionCriteria.length == 0) {
					delete ail[index].apr[indexAPR].locationSelectionCriteria;
				}

				// APR.4, slotSpacingCriteria
				if (aprSeg['APR.4']['APR.4.1'].toString() != "") { ail[index].apr[indexAPR].slotSpacingCriteria = aprSeg['APR.4']['APR.4.1']; }

				// APR.5, fillerOverrideCriteria
				ail[index].apr[indexAPR].fillerSelectionCriteria = [];
				for each (field in aprSeg['APR.5']) {
					...
					if (Object.keys(block).length > 0) {
						ail[index].apr[indexAPR].fillerSelectionCriteria.push(block);
					}
				}
				if (ail[index].apr[indexAPR].fillerSelectionCriteria.length == 0) {
					delete ail[index].apr[indexAPR].fillerSelectionCriteria;
				}

				// clean up APR and increment index
				if (Object.keys(ail[index].apr[indexAPR]).length == 0) {
					delete ail[index].apr[indexAPR];
				} else {
					indexAPR++;
				}
			}
